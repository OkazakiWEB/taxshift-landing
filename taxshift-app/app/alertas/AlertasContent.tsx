    ...
    <AppShell user={user}>
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-sm font-medium transition-all ${
              filter === f.id
                ...
                : 'text-[#6b7280] hover:text-[#23252c]'
            }`}
          >
            {f.label}
            <span
              className={`text-xs rounded-full px-1.5 py-0.5 font-semibold ${
                filter === f.id ? 'bg-[#0d0e11] text-white' : 'bg-[#e5e7eb] text-[#6b7280]'
              }`}
            >
              {f.count}
            </span>
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          subtitle="Não há alertas nesta categoria no momento."
        />
      ) : (
        <div className="space-y-3">
          {filtered.map((alert) => {
            ...
            return (
              <div
                key={alert.id}
                className={`bg-white border border-[#e5e7eb] rounded-xl p-5 shadow-sm transition-all hover:shadow-md ${
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-3 mb-1.5">
                      ...
                      <span
                        className={`text-xs font-medium px-2 py-0.5 rounded-full border flex-shrink-0 ${config.badgeColor}`}
                      >
                        {config.label}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-2 text-xs text-[#9ca3af]">
                        ...
                        <span className="font-medium text-[#6b7280]">{alert.clientName}</span>
